    ...
    for i in range(traj_len):
        ...
        if write_video:
            tf = T.Compose([
                ...
                T.ToTensor(),
                # T.Resize((160, 256), T.InterpolationMode.NEAREST),
                T.Pad((0, 48), fill=0, padding_mode='constant'),
                lambda x: (x.permute(1, 2, 0).numpy() * 255).astype(np.uint8),
            ])
            if video_count % video_skip == 0:
                video_img = []
                for cam_name in camera_names:
                    # im = env.sim.render(height=256, width=256, camera_name=cam_name)[
                    # im = env.sim.render(height=512, width=512, camera_name=cam_name)[
                    im = env.sim.render(height=160, width=256, camera_name=cam_name)[
                        ::-1
                    ]
                    im = tf(im)
...
def playback_dataset(args):
    ...
    if args.n is not None:
        ...
        print(f"playing back {args.n} demos over {len(demos)} available")
        demos = demos[: args.n]
    ...
    for ind in tqdm(range(len(demos))):
        ...
        # maybe dump video
        video_writer = None
        video_path = os.path.join(args.video_dir, ep + '.mp4')
        if write_video:
