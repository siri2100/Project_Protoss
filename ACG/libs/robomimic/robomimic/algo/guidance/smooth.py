...
class Gr00tPolicy_Smooth(Gr00tPolicy):
...
class GR00T_N1_Smooth(GR00T_N1):
...
class FlowmatchingActionHead_Smooth(FlowmatchingActionHead):
    @torch.no_grad()
    def get_action(
        ...
        **kwargs,
        ...
        for t in range(num_steps):
            ...
            # Convert to numpy for scipy filtering (convert BFloat16 to float32 first)
            velocity_np = pred_velocity.cpu().float().numpy()  # (B, T, D)

            # Reshape for batch processing: (B*D, T) to apply filter along T dimension
            reshaped_velocity = velocity_np.transpose(0, 2, 1).reshape(-1, self.action_horizon)  # (B*D, T)

            # Apply Gaussian filter along T dimension for each batch and action dimension
            smoothed_velocity = gaussian_filter1d(reshaped_velocity, sigma=sigma, axis=1)  # (B*D, T)

            # Reshape back to (B, T, D) and convert to torch tensor
            smoothed_velocity = smoothed_velocity.reshape(batch_size, self.config.action_dim, self.action_horizon)
            smoothed_velocity = smoothed_velocity.transpose(0, 2, 1)  # (B, T, D)
            pred_velocity = torch.from_numpy(smoothed_velocity).to(device=device, dtype=pred_velocity.dtype)
