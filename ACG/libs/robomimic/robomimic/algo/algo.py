...
class RolloutPolicy(object):
    ...
    def __call__(self, ob, goal=None, batched=False, *args, **kwargs):
        """
        ...
        ac = self.policy.get_action(obs_dict=ob, goal_dict=goal, *args, **kwargs)
        if not batched:
            ac = ac[0]
        ac = TensorUtils.to_numpy(ac)
        if self.action_normalization_stats is not None:
            if 'env_name' in kwargs:
                action_normalization_stats, action_configs = self.action_normalization_stats

                emb_name = EMBODIMENT_ID_TO_NAME[ENV_NAME_TO_EMBODIMENT_ID[kwargs['env_name']]]
                action_normalization_stats = action_normalization_stats[emb_name]
                action_keys = action_normalization_stats.keys()
                action_config = action_configs[emb_name]
            else:
                action_normalization_stats = self.action_normalization_stats
                action_keys = self.policy.global_config.train.action_keys if 'env_name' not in kwargs else action_normalization_stats.keys()
...
class RolloutPolicyWOLangEncoder(RolloutPolicy):
    def start_episode(self, lang=None):
        """
        Prepare the policy to start a new rollout.
        """
        assert self.lang_encoder is None, "lang_encoder should be None"
        self._ep_lang_emb = lang
        self.policy.set_eval()
        self.policy.reset()
    ...
    def _prepare_observation(self, ob, batched=False):
        ...
        ob = TensorUtils.to_float(ob)
        if self._ep_lang_emb is not None:
            ob['lang_str'] = self._ep_lang_emb  # the lang string is not tensor
        return ob


class RolloutPolicyWOLangEncoderAnalysis(RolloutPolicyWOLangEncoder):

    def reset_stats(self):
        self.states = []
        self.actions = []

    def save_stats(self, save_dir: str, task_name: str):
        timestamp = datetime.datetime.now().strftime("%y%m%d-%H:%M:%S")
        os.makedirs(os.path.join(save_dir, 'stats'), exist_ok=True)
        torch.save(torch.stack(self.states, dim=-2).cpu(), os.path.join(save_dir, 'stats', f'{task_name}_states_{timestamp}.pt'))  # (B, T, D) or (T, D)
        torch.save(torch.stack(self.actions, dim=-2).cpu(), os.path.join(save_dir, 'stats', f'{task_name}_actions_{timestamp}.pt'))  # (B, T, D) or (T, D)
    ...
    def __call__(self, ob, goal=None, batched=False, *args, **kwargs):
        ...
        if env_name is None:
            from robomimic.algo.gr00t_guidance import ROBOCASA_KEY_TO_GR00T_KEY
            states = []
            for k in ROBOCASA_KEY_TO_GR00T_KEY:
                if k in ob and ROBOCASA_KEY_TO_GR00T_KEY[k].startswith('state.'):
                    states.append(ob[k])
            self.states.append(torch.cat(states, dim=-1))  # (B, D)
        else:
            from robomimic.algo.gr00t_guidance_dexmg import DEXMG_KEY_TO_GR00T_KEY
            emb_id = ENV_NAME_TO_EMBODIMENT_ID[env_name]
            states = []
            for k in DEXMG_KEY_TO_GR00T_KEY[emb_id]:
                if k in ob and DEXMG_KEY_TO_GR00T_KEY[emb_id][k].startswith('state.'):
                    states.append(ob[k])
            self.states.append(torch.cat(states, dim=-1))  # (B, D)
        self.actions.append(ac.clone())
