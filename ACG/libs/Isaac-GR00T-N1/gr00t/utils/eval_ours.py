# SPDX-FileCopyrightText: Copyright (c) 2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import os

import matplotlib.pyplot as plt
import numpy as np
from gr00t.data.dataset import LeRobotSingleDataset
from gr00t.model.policy import BasePolicy
from tqdm import tqdm

# numpy print precision settings 3, dont use exponential notation
np.set_printoptions(precision=3, suppress=True)


def calc_mse_for_single_trajectory_single_panda_gripper(
    policy: BasePolicy,
    dataset: LeRobotSingleDataset,
    traj_id: int,
    # modality_keys_state: list,
    modality_keys_action: list,
    steps=300,
    action_horizon=16,
    plot=False,
    plot_path=None,
):
    # state_joints_across_time = []
    gt_action_joints_across_time = []
    pred_action_joints_across_time = []

    pbar = tqdm(total=steps // action_horizon + 1, desc=f'Trajectory {traj_id}')
    for step_count in range(steps):
        data_point = dataset.get_step_data(traj_id, step_count)

        # NOTE this is to get all modality keys concatenated
        # concat_state = data_point[f"state.{modality_keys[0]}"][0]
        # concat_gt_action = data_point[f"action.{modality_keys[0]}"][0]
        # concat_state = np.concatenate(
        #     [data_point[f"state.{key}"][0] for key in modality_keys_state], axis=0
        # )
        concat_gt_action = np.concatenate(
            [data_point[f"action.{key}"][0] for key in modality_keys_action], axis=0
        )

        # state_joints_across_time.append(concat_state)
        gt_action_joints_across_time.append(concat_gt_action)

        if step_count % action_horizon == 0:
            # print("inferencing at step: ", step_count)
            pbar.set_postfix_str(f"inferencing at {step_count}")
            action_chunk = policy.get_action(data_point)
            for j in range(action_horizon):
                # NOTE: concat_pred_action = action[f"action.{modality_keys[0]}"][j]
                # the np.atleast_1d is to ensure the action is a 1D array, handle where single value is returned
                concat_pred_action = np.concatenate(
                    [np.atleast_1d(action_chunk[f"action.{key}"][j]) for key in modality_keys_action],
                    axis=0,
                )
                pred_action_joints_across_time.append(concat_pred_action)
                assert concat_pred_action.shape == concat_gt_action.shape, (
                    f"concat_pred_action shape: {concat_pred_action.shape}, "
                    f"concat_gt_action shape: {concat_gt_action.shape}"
                )
            pbar.update(1)
    pbar.close()

    # plot the joints
    # state_joints_across_time = np.array(state_joints_across_time)
    gt_action_joints_across_time = np.array(gt_action_joints_across_time)
    pred_action_joints_across_time = np.array(pred_action_joints_across_time)[:steps]
    # assert (
    #     state_joints_across_time.shape
    #     == gt_action_joints_across_time.shape
    #     == pred_action_joints_across_time.shape
    # )
    assert gt_action_joints_across_time.shape == pred_action_joints_across_time.shape

    # calc MSE across time
    mse = np.mean((gt_action_joints_across_time - pred_action_joints_across_time) ** 2)
    print("Unnormalized Action MSE across single traj:", mse)

    num_of_joints = gt_action_joints_across_time.shape[1]
    nrows = int(np.ceil(np.sqrt(num_of_joints * 2)))
    ncols = int(np.ceil(num_of_joints / nrows))
    nrows = nrows - 1 if num_of_joints <= (nrows - 1) * ncols else nrows

    if plot:
        # fig, axes = plt.subplots(nrows=num_of_joints, ncols=1, figsize=(8, 4 * num_of_joints))
        fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(8 * ncols, 4 * nrows))
        axes = axes.flatten()

        # Add a global title showing the modality keys
        fig.suptitle(
            f"Trajectory {traj_id} - Modalities: {', '.join(modality_keys_action)}",
            fontsize=16,
            color="blue",
        )

        # for i, ax in enumerate(axes):
        for i in range(num_of_joints):
            ax = axes[i]
            # ax.plot(state_joints_across_time[:, i], label="state joints")
            ax.plot(gt_action_joints_across_time[:, i], label="gt action joints")
            ax.plot(pred_action_joints_across_time[:, i], label="pred action joints")

            # put a dot every ACTION_HORIZON
            for j in range(0, steps, action_horizon):
                if j == 0:
                    ax.plot(j, gt_action_joints_across_time[j, i], "ro", label="inference point")
                else:
                    ax.plot(j, gt_action_joints_across_time[j, i], "ro")

            ax.set_title(f"Joint {i}, MSE: {np.mean((gt_action_joints_across_time[:, i] - pred_action_joints_across_time[:, i]) ** 2):.3f}")
            ax.legend()

        plt.tight_layout()
        if plot_path is not None:
            os.makedirs(os.path.dirname(plot_path), exist_ok=True)
            fig.savefig(plot_path, bbox_inches="tight")
        plt.show()

    return mse
